...
export default function Statistic() {
  const currentLevelCount: number = useAppSelector(
    (state) => state.statistic.currentLevelCount
  );
  const level = useAppSelector(state => state.statistic.level)
  const todos = useAppSelector((state) => state.todo.value)

  const dispatch = useAppDispatch();
  const normalise = (currentLevelCount: any) =>
    ((currentLevelCount - 0) * 100) / (level * 5 - 0);
    ...
    <div className={classes.statBlock}>
      ...
      <div className={classes.lvlBlock}>
        ...
        <LinearProgress
          ...
          variant="determinate"
          value={normalise(currentLevelCount)}
        />
      </div>
    </div>
  );
